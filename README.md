...
整个项目我算是主导者和leader的身份，全程推进着整个项目的流程，一开始并没有什么架构意识，只是按照普通的模块进行文件划分，开始在项目中应用一些架构模式是在9月份看到了一篇关于Android架构的博客，当时心想正好借这个项目练一下这些架构模式就可以充分的进行理解。

一开始了解的是MVC和MVP，然后当时并不知道牛x轰轰的MVVM，进行了一番比较，发现MVC的View和Model解耦不彻底，尤其是在Android里面，整个Activity就相当于是Controller+View,这导致模块视图和逻辑并不能完整的分开，耦合度过高。

想了想，又去看了MVP的架构模式，发现MVP采用了一个叫Presenter的桥接件来代替了MVC的Controller，这个桥接件的存在不允许View和Model直接进行通信了，我发现MVP相对MVC而言解耦更加彻底，但用起来也略有些复杂，对新手不大友好。

因为MVP会创建很多无法复用的接口类，当项目整体非常庞大的时候，这部分接口类会占据一个可怕的文件数目。

但是当时不是很清楚MVVM，看了几篇文章没有怎么搞懂，现在看来，也主要是因为MVVM的使用必须借助Google在17年提供的一个框架，它增强了xml视图的能力，通过ViewModel构造一种通知者的模式，不管是数据还是视图的双向变化都会引起另一项的相应变换，这就是MVVM的牛x之处，但是相应的，MVVM它使用起来过于复杂，对新手极其不友好，想用好MVVM需要有一个适应的时间周期去理解google提供这个框架中的各个组件起到的作用，但或许也正因为它比较复杂，所以不会再出现MVP的缺势，但也正因为它的复杂，我没有在圆月行中采用这个架构模式，一方面是因为初学Android的我难以理解MVVM的妙处，再一个，如果MVVM架构在小型项目中则体现不出它的优势，有点杀鸡焉用牛刀的意思。

最近我开始考虑是否要将整个项目的架构模式进行一次重构，将之前的所有不好的东西全部推翻，然后用MVVM来替代MVP减少整个apk包的大小，减小模块间的耦合性，增强代码的鲁棒性。

我不知道是不是该进行这样的一次重构，因为只有我一个人的话，对整个项目模块的重构会是一个不小的工作量，预计可能要两个周的时间，暂时可能抽不出来这样的空闲时间，所以说，还是将MVVM放在下一个项目里面作为主要架构的模式吧。

**框架的选择**

面试的时候，面试官有问道，你说你作为项目的leader，你为什么选择glide作为图片加载框架呢？为什么又选择Volley作为图片处理框架呢？

一开始其实并不会考虑这么多，就是想作为一个优秀的coder，去将很多推荐的框架用一遍，熟悉熟悉流程，算是对自己的知识体系的一个弥补。

其实不是说选择Volley作为一个图片加载框架，只是因为我对Volley比较熟悉，项目中的网络框架很冗杂，我没有进行整理，从最早期用的Volley过渡到Okhttp3，再过渡到easyhttp，再到最后的retrofit+rxjava，我是一步步走过来亲自感受然后熟悉它们之间的一个异同的，我确实对Volley最为熟悉，甚至看过它的底层源码，知道它的完整流程，甚至自己可以按照自己的想法构造一个小型的Volley。但我也知道easyhttp的易用性，它请求方便，对新手友好，虽然耦合度是严重一些。我又去尝试了Retrofit，它帮我对网络模块强烈解耦，配合rxjava使用简直是神器，但遗憾的是，retrofit+rxjava虽然强解耦，但它用起来比较复杂，对新手不够友好，学习周期要远大于前面的两个框架，我没有办法衡量它们之间的一个选择，只能不同的场景用不同的框架去做，因为框架的选择也是我学习的过程，volley也好，okhttp也罢，它们都存在在我的项目中，我没有对其进行一个更多的重构，一方面是时间成本很高昂，另一方面，圆月行记录的是我成长的脚步，每一个框架的选择也是，但我下一个项目中的网络选择，必然会全部选择retrofit+rxjava。

而glide其实就不用说了，三部曲一走，所有的图片框架没有任何一个能和Glide想提并论，我想说它封装的实在太好了，这三条语句(with.load.into)后面隐藏的是万倍的工作量，我无法不对这样的框架赋予青睐，更何况，Glide占用的资源大小，可比Piccsso占用的小多了，考虑到一个apk体积的大小，我依旧会选择Glide，再一个，glide后面封装了完善的操作，就研究的角度而言，我想选择Glide，有助于源码的阅读能力的培养，我为什么不选择Glide呢？没有理由不选择它。

数据请求框架我用了LitePal，不说性能，单纯我对郭神的敬仰，我就不会选择其它，在一个LitePal功能很完善，又是中国人开发，开发文档的阅读时间大量减少，且郭神的能力自然是不用怀疑，封装的框架Star数有5000+，即使真的出了问题也可以去issues里面反馈，所以选定LitePal作为数据库框架，用过了才知道，LitePal确实很方便，不论是数据的读取还是写入，郭神更是针对LitePal写了几篇博客来进一步简化LitePal的学习成本，我没有理由不选择LitePal。

**三方sdk的选择**

*环信*

可能有人会问，你开发一个项目，用什么三方，大公司不用三方，都自己写，三方不稳定。

可大家不会想到，我一个人能做的事情着实有限，现在的确不是一个人单打独斗拼出整个世界的时代了。

比如圆月行中会有一个社交模块，如果我自己去写一个三方社交服务器，ok，我想那没有什么大的问题，首先为了建立通信，我会找本网络开发的书整个看完（网络课程大三下才开），时间是两个周，然后我开始进行服务器的搭建，数据库的构建，后台接口的设计，又是两个周的时间。

ok，我现在用了四个周完成了整个社交后台的一个搭建，然后当我发现，我不单单需要满足用户间的通信需求，我还要满足群组间的通信，音频通信，视频通信，离线消息的存储，这每一个模块都需要大量的时间和精力，所以我被迫只能选择环信三方sdk构建依赖，这才得以让项目的即时通信模块有了保障。

选择环信因为环信相对其他sdk价格要便宜很多，而我是真的想让圆月行上线，考虑到价格问题，选择了换新。

*高德sdk*

其实一开始选定的不是高德，而是百度地图的sdk，实际用了我才发现，百度地图提供给开发者的权限少之又少，定制性极差，不得已我想到了高德，没有想到高德不愧是专门做地图的公司，高德sdk各方面几乎完全碾压百度sdk，我看完了整个开发文档后，将高德sdk移入了项目当中，后来的很多业务需求，高德也没有让我失望，可定制性极好，这个我一定要好评。
